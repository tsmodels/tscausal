plot.tscausal <- function(x, y = NULL, n_original = 100, ...)
{
  ...
  plot(p$point_distribution, interval_quantiles = c(p$alpha/2, 1 - p$alpha/2), interval_color = "tomato1", gradient_color = "grey", x_axes = FALSE)
  abline(v = as.Date(p$intervention_date), col = "green", lty = 2, lwd = 2)
  ...
  plot(p$cumulative_effect_dist, interval_quantiles = c(p$alpha/2, 1 - p$alpha/2), interval_color = "tomato1", gradient_color = "grey", x_axes = TRUE)
  abline(v = as.Date(p$intervention_date), col = "green", lty = 2, lwd = 2)
